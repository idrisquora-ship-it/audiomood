        ...
        <div className="flex h-full items-center gap-4 px-4">
          <div className="flex items-center gap-3">
            {currentSong.cover_url ? (
              <img src={currentSong.cover_url} alt={currentSong.title} className="h-10 w-10 rounded-md object-cover" />
            ) : (
    ...
    <div className="fixed bottom-0 left-64 right-0 z-50 border-t border-border bg-background/95 backdrop-blur-lg">
      ...
      <div className="flex h-24 items-center justify-between px-6">
        <div className="flex w-1/4 items-center gap-4">
          {currentSong.cover_url ? (
            <img src={currentSong.cover_url} alt={currentSong.title} className="h-16 w-16 rounded-lg object-cover shadow-lg" />
          ) : (
            <div className="flex h-16 w-16 items-center justify-center rounded-lg bg-muted shadow-lg">
              <Music className="h-8 w-8 text-muted-foreground" />
            </div>
          )}
          <div className="min-w-0">
            <p className="truncate font-medium">{currentSong.title}</p>
            <p className="truncate text-sm text-muted-foreground">{currentSong.artist?.display_name}</p>
          </div>
        </div>
        <div className="flex flex-col items-center gap-2">
          <div className="flex items-center gap-4">
            ...
            <Button onClick={togglePlay} className="h-12 w-12 rounded-full gradient-primary glow-primary">
          ...
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            ...
            <Slider value={[currentTime]} max={duration || 100} step={1} onValueChange={([value]) => seek(value)} className="w-64 cursor-pointer" />
        ...
        <div className="flex w-1/4 items-center justify-end gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Slider value={[volume * 100]} max={100} step={1} onValueChange={([value]) => setVolume(value / 100)} className="w-24 cursor-pointer" />
          </div>
